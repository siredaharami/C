...
def file_check():
    ...
    if not os.path.exists(medium):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/Roboto-Medium.ttf",
            medium,
        )
    if not os.path.exists(mono):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/DroidSansMono.ttf",
            mono,
        )
    if not os.path.exists(italic):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/Roboto-Italic.ttf",
            italic,
        )
    if not os.path.exists(fallback):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/Quivira.otf",
            fallback,
        )
...
async def process(msg, user, client, reply, event, replied=None):  # sourcery no-metrics
    ...
    sepcialn = ImageFont.truetype("./temp/assfont.ttf", 41, encoding="utf-16")
    sepcialt = ImageFont.truetype("./temp/assfont.ttf", 35, encoding="utf-16")
    ...
    if replied:
        ...
        if reply.sticker:
            ...
            if sticker.endswith(("tgs")):
                ...
                stdout, stderr = (await _eagleutils.runcmd(cmd))[:2]
...
async def replied_user(draw, tot, text, maxlength, title):
    namefont = ImageFont.truetype("./temp/assfont.ttf", 38)
