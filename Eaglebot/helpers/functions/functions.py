...
async def fileinfo(file):
    ...
    eagle_json = json.loads(x)["media"]["track"]
    dic = {
        "path": file,
        "size": int(eagle_json[0]["FileSize"]),
        "extension": eagle_json[0]["FileExtension"],
    }
    try:
        if "VideoCount" or "AudioCount" or "ImageCount" in cat_json[0]:
            dic["format"] = eagle_json[0]["Format"]
            dic["type"] = eagle_json[1]["@type"]
            if "ImageCount" not in eagle_json[0]:
                dic["duration"] = int(float(eagle_json[0]["Duration"]))
                dic["bitrate"] = int(int(eagle_json[0]["OverallBitRate"]) / 1000)
            if "VideoCount" or "ImageCount" in eagle_json[0]:
                dic["height"] = int(eagle_json[1]["Height"])
                dic["width"] = int(eagle_json[1]["Width"])
...
async def animator(media, mainevent, textevent):
    ...
    eagleOp = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {eagleOp} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(eagleOp)
...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "eaglebot/helpers/styles/impact.ttf"
    font = ImageFont.truetype(font_name, font_size)
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    eagle = await sticcers[0].click("me", hide_via=True)
    if eagle:
        await bot.send_file(int(chat_id), eagle, reply_to=reply_to_id)
        await eagle.delete()
