...
async def song_download(url, event, quality="128k", video=False, title=True):
    ...
    with contextlib.suppress(Exception):
        ...
        if stderr:
            return await eor(event, f"**Error ::** `{stderr}`")
    ...
    await eor(event, f"__Uploading requested {media_type}...__")
...
async def get_group_call(chat):
    if isinstance(chat, Channel):
        result = await eagle(functions.channels.GetFullChannelRequest(channel=chat))
    elif isinstance(chat, Chat):
        result = await eagle(functions.messages.GetFullChatRequest(chat_id=chat.id))
...
async def parse_entity(entity):
    if entity.isnumeric():
        entity = int(entity)
    return await eagle.get_entity(entity)
