...
from Eaglebot.core.decorators import check_owner
from Eaglebot.helpers import humanbytes, progress
from Eaglebot.helpers.utils import _eagleutils
...
# BACK
@eagle.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@eagle.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@eagle.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@eagle.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@eagle.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _eagleutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"{rpath} removed successfully...")


# SEND
@eagle.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@eagle.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@eagle.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@eagle.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
