...
async def download_audio(event):  # sourcery skip: low-code-quality
    ...
    eagleevent = await eor(event, "`Preparing to download...`")
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "eagleuserbot", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await eod(eagleevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(eagleevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("lol_ytv.mp4")
            print(f)
            eaglethumb = pathlib.Path("lol_ytv.jpg")
            if not os.path.exists(eaglethumb):
                eaglethumb = pathlib.Path("lol_ytv.webp")
            if not os.path.exists(eaglethumb):
                eaglethumb = None
            await eagleevent.edit(
                f"`Preparing to upload video:`\
