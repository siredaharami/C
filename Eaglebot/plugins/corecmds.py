...
def plug_checker(plugin):
    plug_path = f"./Eaglebot/plugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./xtraplugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./Eaglebot/assistant/{plugin}.py"
    return plug_path
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by EagleBot) to install it in your bot.",
        "usage": "{tr}install",
    },
)
async def install(event):
    ...
    b = 1
    owo = event.text[9:]
    eagle = await eor(event, "__Installing.__")
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    await event.get_reply_message(),
                    "./Eaglebot/plugins/",  # pylint:disable=E0602
                )
            )
            ...
            await eod(eagle, f"**Failed to Install** \n`Error`\n{str(e)}")
            return os.remove(downloaded_file_name)
