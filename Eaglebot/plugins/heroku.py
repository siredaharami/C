...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        eagle = await eor(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            eagle = "**ConfigVars**:" f"\n\n {variable} = `{heroku_var[variable]}`\n"
            if "eagle_STRING" in variable:
                await eor(
                    var, "eagle String is a Sensetive Data.\nProtected By Eaglebot"
                )
                return
        ...
        eagle = await eor(var, "`Setting information...`")
        if not variable:
            return await eagle.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if "eagle_STRING" in variable:
            await eagle.edit("Successfully Changed")
            return
        ...
        eagle = await eor(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await eagle.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await eagle.edit(f"`{variable}`**  does not exist**")

        await eagle.edit(f"`{variable}`  **successfully deleted**")
...
@eagle.eagle_cmd(
    ...
    info={
        "header": "To Check dyno usage of Eaglebot and also to know how much left.",
        "usage": "{tr}usage",
    },
)
