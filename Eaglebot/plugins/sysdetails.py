...
async def cpu(event):
    ...
    await eor(
        event, f"**[eagle's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    eagleevent = await eor(sysd, "`Fetching system information.`")
    ...
    a, b, c, d = await _eagleutils.runcmd(neo)
    result = str(a) + str(b)
    await eor(eagleevent, "**Neofetch Result:** `" + result + "`")
