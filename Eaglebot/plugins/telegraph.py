...
from . import eagle, mention, reply_id
...
async def ctg(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eagle(unblock("chotamreaderbot"))
            msg_flag = await conv.send_message(urls[0])
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        if response.text.startswith(""):
            await eor(eagleevent, "Am I Dumb Or Are U Dumb?")
        else:
...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        ms = (end - start).seconds
        eagle = f"https://telegra.ph/{response['path']}"
        await eagleevent.edit(
            f"**✓ Uploaded to :-** [telegraph]({eagle})\
                 \n**✓ Uploaded in {ms} seconds.**\
                 \n**✓ Uploaded by :-** {mention}\
                 \n**✓ Telegraph :-** `{eagle}`",
            link_preview=True,
