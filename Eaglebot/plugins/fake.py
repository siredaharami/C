...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, eagleevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await eod(eagleevent, f"__{e}__", time=10)
    await eagleevent.edit("`Promoted Successfully! Now gib Party`")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 5122474448:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 99$ to my master__ [EagleBoy](https://t.me/ll_BAD_MUNDA_ll) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
