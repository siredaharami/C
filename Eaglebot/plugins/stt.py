...
async def _(event):
    ...
    eagleevent = await eor(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await eagleevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await eagleevent.edit(r["error"])
    ...
    await eagleevent.edit(string_to_show)
