...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = (await eagle(GetFullUserRequest(LASTFM_.USER_ID))).full_user
            ...
            if (
                ...
                try:
                    ...
                    await eagle(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await eagle(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await eagle(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await eagle.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await eagle.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
...
async def lastbio(lfmbio):
    ...
    if arg == "on":
        ...
        if not LASTFM_.LASTFMCHECK:
            ...
            await eor(lfmbio, LFM_BIO_ENABLED)
            await sleep(4)
            await get_curr_track(lfmbio)
        else:
            await eor(lfmbio, LFM_BIO_RUNNING)
    elif arg == "off":
        LASTFM_.LASTFMCHECK = False
        LASTFM_.RUNNING = False
        await lfmbio.client(UpdateProfileRequest(about=DEFAULT_BIO))
        await eor(lfmbio, LFM_BIO_DISABLED)
    else:
        await eor(lfmbio, LFM_BIO_ERR)
...
async def lastlog(lstlog):
    ...
    if arg == "on":
        ...
        await eor(lstlog, LFM_LOG_ENABLED)
    elif arg == "off":
        LASTFM_.LastLog = False
        await eor(lstlog, LFM_LOG_DISABLED)
    else:
        await eor(lstlog, LFM_LOG_ERR)
