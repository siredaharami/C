...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec eagle stringsetup.py",
    },
)
async def _(event):
    ...
    eagleevent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    EagleUSer = await event.client.get_me()
    curruser = EagleUSer.username or "EagleUserBot"
    uid = os.geteuid()
    if uid == 0:
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('EagleUserBot')",
    },
)
async def _(event):
    ...
    if "session" in cmd:
        return await eor(
            event, "`This Is A Sensitive Data.So Its Protected By Eaglebot.`"
        )
    if "EAGLE_STRING" in cmd:
        return await eor(
            event, "`This Is A Sensitive Data.So Its Protected By Eaglebot.`"
        )
    cmd = (
    ...
    eagleevent = await eor(event, "`Running ...`")
