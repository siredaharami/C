...
autopic_path = os.path.join(os.getcwd(), "Eaglebot", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "Eaglebot", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "Eaglebot", "photo_pfp.png")
...
async def autopicloop():
    ...
    if AUTOPICSTART and DEFAULT_PIC is None:
        if BOTLOG:
            return await eagle.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await eagle.upload_file(autophoto_path)
        try:
            await eagle(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await eagle.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await eagle(
                    functions.photos.DeletePhotosRequest(
                        await eagle.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await eagle(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await eagle(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await eagle(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await eagle(functions.photos.UploadProfilePhotoRequest(file))
        await _eagleutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(CHANGE_TIME)
...
eagle.loop.create_task(autopfp_start())
eagle.loop.create_task(autopicloop())
eagle.loop.create_task(digitalpicloop())
eagle.loop.create_task(bloom_pfploop())
eagle.loop.create_task(autoname_loop())
eagle.loop.create_task(autobio_loop())
eagle.loop.create_task(custompfploop())
