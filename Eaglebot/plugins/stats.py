...
async def stats(event):  # sourcery no-metrics
    ...
    eagle = await eor(event, STAT_INDICATION)
...
async def stats(event):  # sourcery no-metrics
    eaglecmd = event.pattern_match.group(1)
    eagleevent = await eor(event, STAT_INDICATION)
    start_time = time.time()
