...
from . import BOTLOG, BOTLOG_CHATID, Config, eagle, reply_id
...
PATH = "Eaglebot/cache/spotify_database.json"
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await eagle.send_message(BOTLOG_CHATID, string)
                else:
            ...
            # need to change anything
                ...
                if save_spam("spotify", False):
                    ...
                    await eagle.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await eagle(GetFullUserRequest(eagle.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if not new_bio == bio:
                            try:
                                await eagle(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio:
                        if save_spam("telegram", True):
                            ...
                            await eagle.send_message(BOTLOG_CHATID, to_send)
                # not to_insert means no playback
                    ...
                    # back to the original one
                        ...
                        await eagle(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await eagle(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
        ...
        USER_INITIAL_BIO["bio"] = (
            (await eagle(GetFullUserRequest(eagle.uid))).full_user
        ).about or ""
...
def telegraph_lyrics(tittle, artist):
    ...
    try:
        response = telegraph.create_page(
            "Lyrics",
            html_content=result,
            author_name="Eaglebot",
            author_url="https://t.me/PBX_CHAT",
        )
...
def file_check():
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/pics/main.jpg",
            logo,
        )
    if not os.path.exists(font_mid):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/assfont.ttf?raw=true",
            font_mid,
        )
    if not os.path.exists(font_bold):
        urllib.request.urlretrieve(
            "https://github.com/Badhacker98/EAGLEPLUGIN/blob/Bad/Eaglebot/helpers/resources/fonts/Roboto-Italic.ttf?raw=true",
            font_bold,
        )
...
async def spotify_now(event):
    ...
    if link:
        ...
        if not url(link) and "spotify" not in link:
            return await eod(eagleevent, "**Give me a correct link...**")
    elif not link:
        if not await sp_var_check(event):
            return
        r = sp_data("https://api.spotify.com/v1/me/player/currently-playing")
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eagle(unblock("Gaana_MusicBot"))
            purgeflag = await conv.send_message("/start")
        ...
        await eagleevent.delete()
        if cmd == "i":
            songg = await eagle.send_file(BOTLOG_CHATID, song)
            fetch_songg = await eagle.tgbot.get_messages(BOTLOG_CHATID, ids=songg.id)
            btn_song = await eagle.tgbot.send_file(
                BOTLOG_CHATID, fetch_songg, buttons=Button.url("🎧 Spotify", link)
            )
            fetch_btn_song = await eagle.get_messages(BOTLOG_CHATID, ids=btn_song.id)
