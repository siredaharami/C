...
from ..helpers.utils import _format, _eagleutils
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "usage": "{tr}ls <path>",
        "examples": "{tr}ls Eaglebot",
    },
)
async def ls(event):  # sourcery no-metrics
    "To list all files and folders."
    eagle = "".join(event.text.split(maxsplit=1)[1:])
    path = eagle or os.getcwd()
    if not os.path.exists(path):
        await eor(
            event,
            f"there is no such directory or file with the name `{eagle}` check again",
        )
        return
    path = Path(eagle) if eagle else os.getcwd()
...
async def rem(event):
    ...
    eagle = event.pattern_match.group(1)
    if eagle:
        path = Path(eagle)
    else:
    ...
    if not os.path.exists(path):
        ...
        return
    eaglecmd = f"rm -rf '{path}'"
    if os.path.isdir(path):
        await _eagleutils.runcmd(eaglecmd)
...
async def mkdir(event):
    ...
    try:
        await _eagleutils.runcmd(f"mkdir {original}")
...
async def cpto(event):
    ...
    await asyncio.sleep(2)
    try:
        await _eagleutils.runcmd(f"cp -r {original} {location}")
