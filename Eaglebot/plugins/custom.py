    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                USERINFO = await eagle.get_entity(legebd.uid)
                FULL_USERINFO = (await eagle(GetFullUserRequest(eagle.uid))).full_user
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await eagle.get_profile_photos(eagle.uid)
                    myphoto = await eagle.download_media(photos[0])
                ...
                if not vinfo and vname == "ALIVE_TEMPLATE":
                    return await eod(event, "Check https://raw.githubusercontent.com/Badhacker98/EAGLEPLUGIN/Bad/Eaglebot/alive/alive.text ")
                if not vinfo:
                    return await eod(
                        event,
...
@eagle.eagle_cmd(
    ...
    info={
        "header": "To customize your EagleUserBot.",
        "options": {
...
async def custom_EagleUserBot(event):
    "To customize your EagleUserBot."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
