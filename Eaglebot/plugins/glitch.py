...
from ..helpers.utils import _eagletools, _eagleutils, reply_id
...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    eagleinput = int(eagleinput) if eagleinput else 2
    glitch_file = await _eagletools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, eagleinput, color_offset=True)
        ...
        glitch_img = glitcher.glitch_image(
            img, eagleinput, color_offset=True, gif=True
        )
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
        ...
        EAGLE = await event.client.send_file(event.chat_id, glitched, reply_to=swtid)
