...
async def gpromote(eagleevent):
    "To promote user in every group where you are admin(have a right to promote)."
    i = 0
    await eagleevent.get_sender()
    me = await eagleevent.client.get_me()
    eagle = await eor(eagleevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await eagleevent.get_chat()
    if eagleevent.is_private:
        user = eagleevent.chat
        rank = eagleevent.pattern_match.group(1)
    else:
        eagleevent.chat.title
    try:
        user, rank = await get_full_user(eagleevent)
    except:
        pass
    if me == user:
        await eagle.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "eagle"
    except:
        return await eagle.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await eagleevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await eagleevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await eagle.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await eagle.edit(f"**Reply to a user !!**")
    await eagle.edit(
        f"[{user.first_name}](tg://user?id={user.id}) **Was Promoted Globally In** `{i}` **Chats !!**"
    )
    await eagleevent.client.send_message(
        BOTLOG_CHATID,
...
async def gdemote(eagleevent):
    "To demote user in that group where you promote person to admin."
    i = 0
    await eagleevent.get_sender()
    me = await eagleevent.client.get_me()
    eagle = await eor(eagleevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    if eagleevent.is_private:
        user = eagleevent.chat
    ...
    try:
        ...
        pass
    if me == user:
        await eagle.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "eagle"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await eagleevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await eagle.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await eagle.edit(f"**Reply to a user !!**")
...
async def loban(event):
    ...
    eagle = await eor(event, "`Ungban in progress...`")
    if event.is_private:
        user = await event.get_chat()
        reason = event.pattern_match.group(1)
