...
from Eaglebot import BOTLOG_CHATID, eagle
...
from ..helpers.utils import _eagletools, reply_id
...
async def maccmd(event):  # sourcery no-metrics
    ...
    eagleevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await _eagletools.media_to_pic(eagleevent, reply, noedits=True)
        if imag[1] is None:
            return await eod(
        ...
        return await eod(eagleevent, f"**Error in identifying image:**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    eagleinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    if not reply:
    ...
    meme = os.path.join("./temp", "eaglememe.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "Eaglebot/helpers/styles/impact.ttf"
    else:
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await eagleevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
        arg = f"Eaglebot/helpers/styles/{input_str}"
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not eagleinput else eagleinput
...
async def memes(event):
    ...
    eagleinput = event.pattern_match.group(1)
    eagleinput = 50 if not eagleinput else int(eagleinput)
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "`Reply to supported Media...`")
    san = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    ...
    try:
        await crop(meme_file, outputfile, eagleinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not eagleinput:
        eagleinput = "50"
    if ";" in str(eagleinput):
        eagleinput, colr = eagleinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, eagleinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
