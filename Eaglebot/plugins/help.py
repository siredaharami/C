...
async def grpinfo():
    outstr = "**Plugins in Eaglebot are:**\n\n"
    outstr += f"**👨‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra", "useless"]
    for eagle in category:
        plugins = GRP_INFO[eagle]
        outstr += f"**{hemojis[eagle]} {eagle.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Eaglebot are :**\n\n"
...
@eagle.eagle_cmd(
    ...
    info={
        "header": "To get guide for Eaglebot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of Eaglebot are :")
