...
from . import _eagletools, convert_toimage, eagle, soft_deEmojify
...
async def ocr(event):
    ...
    eagleevent = await eor(event, "`Reading...`")
    ...
    try:
        ...
        await eagleevent.edit("`Couldn't read it.. you sure this readable !?`")
    test_file = await ocr_space_file(filename=output_file, language=lang_code)
    try:
        ParsedText = test_file["ParsedResults"][0]["ParsedText"]
    except BaseException:
        await eagleevent.edit("`Couldn't read it.`\n`I guess I need new glasses.`")
    else:
        if cmd == "":
