...
async def get_users(show):
    ...
    eagleevent = await eor(show, "`getting users list wait...`  ")
    try:
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @HEROKUBIN_01",
    },
)
async def info(event):
    "To get group information"
    eagleevent = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, eagleevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await eagleevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await eagleevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, eagleevent):
    ...
    try:
        ...
        try:
            ...
            await eagleevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await eod(eagleevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
