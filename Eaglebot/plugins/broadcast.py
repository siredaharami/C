...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @PBX_CHAT just a testmessage",
    },
)
async def eaglebroadcast_add(event):
...
async def eaglebroadcast_add(event):
    ...
    eagleinput_str = event.pattern_match.group(1)
    if not eagleinput_str:
        return await eod(
            event,
            "In Which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = eagleinput_str.lower()
...
async def eaglebroadcast_list(event):
    ...
    await eor(eagleevent, finaloutput)
...
async def eaglebroadcast_send(event):
    ...
    await eod(eagleevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def eaglebroadcast_remove(event):
...
async def eaglebroadcast_delete(event):
    "To delete a category completely."
    eagleinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(eagleinput_str)
    if check1 < 1:
        return await eod(
            event,
            f"Are you sure that there is category {eagleinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(eagleinput_str)
        await eor(
            event,
            f"Successfully deleted the category {eagleinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await eod(
            ...
            try:
                if chat != -1001551357238:
                    await bot.send_message(
                        chat,
                        f"⣿⣿⣿⣿⣿⣍⠀⠉⠻⠟⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⠓⠀⠀⢒⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⡿⠃⠀⠀⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠿⣿\n⣿⡿⠋⠋⠀⠀⠀⠀⠀⠀⠈⠙⠻⢿⢿⣿⣿⡿⣿⣿⡟⠋⠀⢀⣩\n⣿⣿⡄⠀⠀⠀⠀⠀⠁⡀⠀⠀⠀⠀⠈⠉⠛⢷⣭⠉⠁⠀⠀⣿⣿\n⣇⣀. INDIA🇮🇳INDIA🇮🇳⠆⠠..⠘⢷⣿⣿⣛⠐⣶⣿⣿\n⣿⣄⠀⣰⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠐⢀⣠⣿⣿⣿⣾⣿⣿⣿\n⣿⣿⣿⣿⠀⠀⠀⠀⡠⠀⠀⠀⠀⠀⢀⣠⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠄⠀⣤⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⡄⠀⠀⠀⠀⠀⣠⣤⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⠀⠀⠂⠀⠀⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣇⠀⠀⠀⢠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⡆⠀⢀⣼⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⣿⣦⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n\n[нαρργ ιи∂ρєи∂єиϲє ∂αγ🇮🇳](https://t.me/PBX_CHAT)",
                    )
                    lol += 1
