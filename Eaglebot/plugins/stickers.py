...
from ..helpers.utils import _eagletools
...
def pack_name(userid, pack, is_anim, is_vid):
    if gvarstatus("CUSTOM_STICKER_SETNAME") is not None:
        ...
        return f"Eaglebot_{userid}_{pack}_anim"
    elif is_vid:
        return f"Eaglebot_{userid}_{pack}_vid"
    else:
        return f"Eaglebot_{userid}_{pack}"
...
async def delpack(eagleevent, conv, args, packname):
    try:
        await conv.send_message("/delpack")
    except YouBlockedUserError:
        await eagle(unblock("stickers"))
        await conv.send_message("/delpack")
...
async def add_to_pack(
    ...
    while ("50" in x.message) or ("120" in x.message) or vtry:
        ...
        await eagleevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            eagleevent = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size / 1024 > 255:
                            eagleevent = await eor(
                                args, "__⌛ File size big,,, Downloading..__"
                            )
                            sticker = await animator(message, args, eagleevent)
                            await eor(eagleevent, f"`{random.choice(KANGING_STR)}`")
                        else:
                            eagleevent = await eor(
                ...
                eagleevent = await eor(args, "__⌛ Downloading..__")
                sticker = await animator(message, args, eagleevent)
                await eor(eagleevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await eagleevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await eagleevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(args):  # sourcery no-metrics
    ...
    eagle = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await eod(eagleevent, "`Unsupported File!`")
            return
        if photo:
            splat = ("".join(args.text.split(maxsplit=1)[1:])).split()
            emoji = emoji or "⚜"
            ...
            if (
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, EAGLEBOTname = await newpacksticker(
                        eagleevent,
                        conv,
                        cmd,
                        args,
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, EAGLEBOTname = await add_to_pack(
                        eagleevent,
                        conv,
                        args,
                        packname,
            ...
            if EAGLEBOTname is None:
                return
            if EAGLEBOTname not in blablapacks:
                blablapacks.append(EAGLEBOTname)
...
async def lol(args):
    ...
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"EagleOp_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
    ...
    await eagleevent.edit("`Hold on, making sticker...`")
    async with args.client.conversation("@Stickers") as conv:
        otherpack, packname, emoji = await newpacksticker(
            eagleevent,
            conv,
...
async def pic2packcmd(event):
    ...
    eagleevent = await eor(event, "__🔪Cropping and adjusting the image...__")
    try:
    ...
    image = await _eagletools.media_to_pic(eagleevent, reply, noedits=True)
    if image[1] is None:
        return await eod(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    eagleevent = await event.edit("__Making the pack.__")
    async with event.client.conversation(chat) as conv:
        ...
        for im in images:
            ...
            i += 1
            await eagleevent.edit(f"__Making the pack.\nProgress: {i}/{len(images)}__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await eagleevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    eagleevent = await eor(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await eod(eagleevent, "`No results found :(.`", 5)
