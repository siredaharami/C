...
async def spam_function(event, EAGLE, lol, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(lol[0])
    if len(lol) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await EAGLE.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and EAGLE.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            EAGLE = await event.client.send_file(
                event.chat_id, EAGLE, caption=EAGLE.text
            )
        ...
        if BOTLOG:
            ...
            EAGLE = await event.client.send_file(BOTLOG_CHATID, EAGLE)
            await _eagleutils.unsavegif(event, EAGLE)
        return
    elif event.reply_to_msg_id and EAGLE.text:
        spam_message = EAGLE.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    EAGLE = await event.get_reply_message()
    ...
    await spam_function(event, EAGLE, lol, sleeptimem, sleeptimet)
