...
async def tmuter(event):  # sourcery no-metrics
    ...
    eagleevent = await eor(event, "`muting....`")
    user, reason = await get_user_from_event(event, eagleevent)
    if not user:
        return
    if not reason:
        return await eagleevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    swttime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(eagleevent, swttime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await eagleevent.edit("Sorry, I can't mute myself")
    try:
        await eagleevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    eagleevent = await eor(event, "`banning....`")
    user, reason = await get_user_from_event(event, eagleevent)
    if not user:
        return
    if not reason:
        return await eagleevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    swttime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await eagleevent.edit("Sorry, I can't ban myself")
    await eagleevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
        ...
        return await eagleevent.edit(NO_PERM)
