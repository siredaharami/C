...
from ..helpers.utils import _eagletools, get_user_from_event, reply_id
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic Eagleuserbot.", "{tr}qpic -b Eagleuserbot."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    eagleevent = await eor(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "Eagleuserbot.Webp"
        img.save(output, "webp")
    else:
        output.name = "Eagleuserbot.png"
        img.save(output, "PNG")
    output.seek(0)
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": ["{tr}fq @PBX_CHAT hello bad boys and girls"],
    },
)
async def stickerchat(owoquotes):
    ...
    eagleevent = await eor(owoquotes, "`Making quote...`")
    if cmd in ["rq", "q"]:
        try:
            user = (
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    eagleevent = await eor(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=7396541413)
            )
            if messages_id != []:
                ...
                return await eod(
                    eagleevent, "`I guess you have used a invalid syntax`"
                )
