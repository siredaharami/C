...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": ["{tr}hbase en Eagleuserbot", "{tr}hbase de Q2F0dXNlcmJvdA=="],
    },
)
    ...
    if event.pattern_match.group(1) == "en":
        if string:
            ...
            if mediatype is None:
                ...
                eagleevent = await eor(event, "`Encoding ...`")
                c_time = time.time()
        ...
        await eor(
            eagleevent, result, file_name="encodedfile.txt", caption="It's Encoded"
        )
    else:
        try:
            lething = str(
