...
from . import BOTLOG, BOTLOG_CHATID, eor, eagle, logging, spamwatch
...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        eagleadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not eagleadmin:
            return
        eaglebanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            eaglegban = gbanned(user.id)
            if eaglegban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{eaglegban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not eaglebanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await eagleevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await eagleevent.edit("`CAS check failed`")
        return
    await eagleevent.edit(text)
...
async def spamchecker(event):
    ...
    try:
        ...
        await eagleevent.edit("`spamwatch check failed`")
        return
    await eagleevent.edit(text)
