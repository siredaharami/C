...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        eagleevent = await eor(event, f"`Searching lyrics for {songinfo}...`")
        lyrics = await LyricsGen.lyrics(event, song, artist)
        if lyrics is None:
            return await eagleevent.edit(f"Song **{songinfo}** not found!")
        result = f"**Search query**: \n`{songinfo}`\n\n```{lyrics}```"
    else:
        eagleevent = await eor(event, f"`Searching lyrics for {query}...`")
