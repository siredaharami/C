...
async def lol(event):
    ...
    try:
        await eagle(functions.channels.JoinChannelRequest(bol))
...
@eagle.eagle_cmd(
    ...
    info={
        ...
        "examples": "{tr}inviteall @PBX_CHAT",
        "note": "⚠️ If u using this cmd i am not responsible for ur id ban or delete",
    },
)
async def get_users(event):
    legen_ = event.text[10:]
    eagle_chat = legen_.lower
    restricted = ["@HEROKUBIN_01", "@PBX_CHAT"]
    EAGLE = await eor(event, f"**Inviting members from** {legen_}")
    if eagle_chat in restricted:
        return await EAGLE.edit(event, "You can't Invite Members from there.")
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await EAGLE.edit("`processing...`")
    else:
        await EAGLE.edit("`processing...`")
    if event.is_private:
        return await EAGLE.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"
    await EAGLE.edit(
        "**⚜️[Terminal Status](https://t.me/PBX_CHAT)**\n\n`👨‍💻Inviting Users.......`"
    )
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            if error.startswith("Too"):
                return await EAGLE.edit(
                    f"**Terminal Finished With Error**\n(`May Got Limit Error from telethon Please try agin Later`)\n**Error** : \n`{error}`\n\n• Invited `{s}` people \n• Failed to Invite `{f}` people"
                )
            tol = f"@{user.username}"
            lol = tol.split("`")
            await eagle(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await EAGLE.edit(
...
async def get_users(event):
    ...
    f = 0
    error = "None"
    await EAGLE.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            if error.startswith("Too"):
                return await EAGLE.edit(
            ...
            tol = user.id
            await eagle(InviteToChannelRequest(channel=event.chat_id, users=[tol]))
            s = s + 1
            await EAGLE.edit(
                f"**Terminal Running...**\n\n• Invited `{s}` people \n• Failed to Invite `{f}` people\n\n**× LastError:** `{error}`"
            )
