...
class EagleGloballist(BASE):
    __tablename__ = "Eagleglobal_list"
    ...
    def __repr__(self):
        return "<Eagle global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, EagleGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


EagleGloballist.__table__.create(checkfirst=True)

EAGLEGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with EAGLEGLOBALLIST_INSERTION_LOCK:
        broadcast_group = EagleGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with EAGLEGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(EagleGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with EAGLEGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(EagleGloballist.keywoard)
            .filter(EagleGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(EagleGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(EagleGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(EagleGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(EagleGloballist).all()
        for x in all_groups:
