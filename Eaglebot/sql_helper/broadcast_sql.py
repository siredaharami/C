...
class EagleBroadcast(BASE):
    __tablename__ = "eaglebroadcast"
    ...
    def __repr__(self):
        return "<Eagle Broadcast channels '%s' for %s>" % (
            self.group_id,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, EagleBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


EagleBroadcast.__table__.create(checkfirst=True)

EAGLEBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with EAGLEBROADCAST_INSERTION_LOCK:
        broadcast_group = EagleBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with EAGLEBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(EagleBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with EAGLEBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(EagleBroadcast.keywoard)
            .filter(EagleBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(EagleBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(EagleBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(EagleBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(EagleBroadcast).all()
        for x in all_groups:
