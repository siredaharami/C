...
class Eagle_GlobalCollection(BASE):
    __tablename__ = "Eagle_globalcollection"
    ...
    def __repr__(self):
        return "<Eagle Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Eagle_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Eagle_GlobalCollection.__table__.create(checkfirst=True)

EAGLE_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with EAGLE_GLOBALCOLLECTION:
        keyword_items = Eagle_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with EAGLE_GLOBALCOLLECTION:
        if keyword_items := SESSION.query(Eagle_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with EAGLE_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Eagle_GlobalCollection.keywoard)
            .filter(Eagle_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Eagle_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Eagle_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Eagle_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Eagle_GlobalCollection).all()
        for x in all_groups:
