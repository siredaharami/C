...
class Gdrive(BASE):
    __tablename__ = "eaglegdrive"
    eagle = Column(String(50), primary_key=True)
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.eagle == str(folder_id))
...
def rmparent_id(folder_id):
    if note := SESSION.query(Gdrive).filter(Gdrive.eagle == folder_id):
        note.delete()
        SESSION.commit()
